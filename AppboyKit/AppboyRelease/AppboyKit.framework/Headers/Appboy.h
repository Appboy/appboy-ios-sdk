...
#define APPBOY_SDK_VERSION @"2.0" // April 11, 2013 build
#endif

@protocol AppboyDelegate;
...
/*!
 * Disable Appboy for future sessions.  Note: This does not affect the current session; the current session will finish.
 */
 ...
 * See AppboyUser.h and changeUser:userId below.
 */
@property (nonatomic, retain) AppboyUser *user;

/*!
 * This delegate is sent a AppboyDelegateStatusChange message after Appboy has initialized and successfully communicated
 * with the server.
 */
@property (nonatomic, assign) id <AppboyDelegate> delegate;
 ...
 * 
 * - Put the NUI initialization code in your App Delegate and main.m.
 *
 * - Create a style sheet called NUIStyle.nss.
 * 
 * - set the property below to TRUE.
 * 
 * If useNUITheming is FALSE, NUI is ignored completely whether or not it's integrated into your app.  Note that
...
/*!
 * The total number of currently active cards displayed in any stream view. Cards are
 * counted only once even if they appear in multiple stream views.
 */
 ...
 * A "view" happens when a card becomes visible in the stream view.  This differentiates
 ...
 *
 * Cards are counted only once even if they appear in multiple stream views or across multiple devices.
 */
 ...
 *
 * <pre>
 * [[Appboy sharedInstance] logAppEvent:@"clicked_button"];
 * </pre>
 */
 ...
 *
 * <pre>
 * [[Appboy sharedInstance] logAppPurchase:@"powerups" priceCents:99];
 * </pre>
 */
 ...
 */
typedef enum {
  AppboySocialNetworkFacebook = 1 << 0,
  AppboySocialNetworkTwitter  = 1 << 1
}AppboySocialNetwork;

/*!
 * @param socialNetworks An AppboySocialNetwork indicating the network that you wish to access.
 ...
 */
- (void) promptUserForAccessToSocialNetwork:(AppboySocialNetwork)socialNetwork;

/*!
 * @param socialNetwork An AppboySocialNetwork indicating the network that you wish to access.
 ...
 */
- (void) logSocialShare:(AppboySocialNetwork)socialNetwork;
 ...
 * - The keyboard is up
 * - A stream view is being displayed as the result of a prior slideup being tapped
 ...
 * - A slideup tap-initiated stream view closes
 ...
 *
 * Second, when a slideup is tapped, the default action is to open an ABKStreamViewControllerModalContext.
 * If slideupDelegate is set, however, the ABKStreamViewControllerModalContext is not opened;
...
/*!
 * This delegate method is called after Appboy has initialized and successfully communicated
 * with the server.
 */
